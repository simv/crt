...
- `npm run screenshots` — regenerates the five README screenshots in `docs/images/` (`packages/server/e2e/screenshots.spec.ts` under `playwright.screenshots.config.ts`: the fixture app on :3998 and a `CRT_SESSION_STUB=1` embedded `crt serve` on :4490 from the scratch project `e2e/.project/screenshots`; 1280 × 800 at DPR 2, light, animations disabled, waits on the overlay's own signals — PRD-polish F-116, N-27). Needs `npm run build` first. By hand only, never in CI; `docs/images/README.md` says what varies between runs. `test/docs-images.test.ts` checks the images exist and stay under budget.
