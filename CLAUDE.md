...
Read `docs/PRD.md`, `docs/PRD-providers.md`, `docs/PRD-setup.md`, `docs/PRD-embedded.md`, `docs/PRD-polish.md` and `docs/PRD-chat.md` before doing anything non-trivial. The first defines scope, requirement IDs (F-n, N-n), milestones (M1–M5) and the project definition of done; the second amends it for v0.2 (provider-agnostic intake: F-42…F-64, N-7…N-13, milestones M6–M11) and its §9 lists which v1.0 statements and CLAUDE.md lines change as each milestone lands; the third amends both for v0.3 setup and first run (guided `crt [target]`, `crt doctor`, `crt setup`, login preflight, arrival UX: F-69…F-90, N-14…N-17, milestones M12–M14) with its own §9 list; the fourth amends all three for v0.4 embedded mode (the CRT server without a proxy by default, a dev-only loader and package entries in the app, explicit `crt init` with a CRT section for agents, the production guarantee: F-91…F-110, N-18…N-22, milestones M15–M18) with its own §9 list; the fifth amends all four for v0.6 polish (the brand mark, the landing page as a status page with live `crt doctor` rows, the README as a front page with the reference moved to `docs/`, reproducible screenshots, release 0.6.0: F-112…F-118, N-23…N-27, milestones M20–M24) with its own §9 list; the sixth amends all five for v0.7 compact chat (the first bubble shows the developer's words and the agent's proposal folds to its restatement and item count, the full text one click away behind a pill, nothing the agent receives changed: F-119…F-121, N-28…N-30, milestones M25–M27) with its own §9 list. Requirement IDs are referenced in code comments, tests, commits and PRs.

## What this is

A local CRT server + in-page overlay loaded by a dev-only integration in the app (or by a proxy, `crt proxy`) that lets a developer annotate their running site and talk to a coding agent in the page (Claude Code by default; the agent is a *provider*, resolved per PRD-providers F-43; the panel shows the developer's words and the agent's proposal folded, the full text one click away (PRD-chat F-119, F-120)); the agent writes a self-contained task file to `.crt/tasks/` that any later session can complete with `/crt:next`. One repo, three deliverables: `packages/server` (npm `claude-review-tool`, bin `crt`), `packages/overlay` (bundled into the server), `plugin/` (Claude Code plugin, marketplace at repo root).
...
- `npm run screenshots` — regenerates the five README screenshots in `docs/images/` (`packages/server/e2e/screenshots.spec.ts` under `playwright.screenshots.config.ts`: the fixture app's `/shop` page (the trial shop, ported into `e2e/fixture/server.mjs`) on :3998 and a `CRT_SESSION_STUB=1` embedded `crt serve` on :4490 from the scratch project `e2e/.project/screenshots`; 800 × 600 at DPR 2, light, animations disabled, waits on the overlay's own signals — PRD-polish F-116, N-27). Needs `npm run build` first. By hand only, never in CI; `docs/images/README.md` says what varies between runs. `test/docs-images.test.ts` checks the images exist and stay under budget.
...
This repo dogfoods CRT. Work items are `.crt/tasks/CRT-NNNN-*.md` in the PRD F-32 format; statuses run `backlog → in_progress → review → done` (or `blocked`). `/crt:tasks` lists them, `/crt:next [ID]` takes one to an open PR, `/crt:done <ID>` closes it once the PR is reviewed: the done commit is the **last commit on the PR branch**, then the PR merges — never a second PR (PRD-chat F-122; the flow is claim → build → commit → review → done → commit → merge). Working a task by hand: claim it first (status → `in_progress`, Log entry), append to its Log as you go, set `review` when its Definition of Done is fully ticked, and close it the same way: `done` + `— done; closed in <PR URL>` in the Log as the branch's last commit, then merge. A task is finished only when it is `done`, its branch is merged to `main`, and no PR is open. `.crt/tasks/README.md` is generated by `crt tasks` — never hand-edit it.
...
- Don't hand-edit `docs/images/*.png` — `npm run screenshots` regenerates them (PRD-polish F-116). The four SVGs there are hand-drawn: fixed colours only, a light and a dark file each.
