...
1. Build the command. `crt` is `npx --no crt` if the project has it installed, else `npx -y claude-review-tool@latest`:
   ...
   ```
   crt serve --open                       # no argument: CRT detects the dev server (PRD F-1)
   crt serve --open --target $ARGUMENTS   # argument given: 3000, localhost:3000 or a full URL
   ```
...
2. Wait for readiness, up to 30 s: the server prints one line
   ...
   ```
   CRT ready at http://localhost:4400 → http://localhost:3000 (project: C:\my-app, 3 tasks)
   ```

   If you cannot read the background output, poll `curl -s http://localhost:4400/__crt/health` (use the port from `.crt/config.json` if it is not 4400); it answers `{"ok":true,"target":…,"projectRoot":…}` once up.
