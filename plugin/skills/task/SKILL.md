...
1. Run `npx --no crt task $ARGUMENTS` (the project's own install), else `npx -y claude-review-tool@0.7 task $ARGUMENTS`; it prints the whole file. If neither works, read `.crt/tasks/$ARGUMENTS-*.md` directly. If the CLI says `no task …`, list the IDs that do exist (`Glob .crt/tasks/CRT-*.md`) and stop.
