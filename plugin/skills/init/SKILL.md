...
`crt` below is `npx --no crt` when the project has it installed (a devDependency, a workspace, or a global install), else `npx -y claude-review-tool@0.4`. The fallback may have to download the package (~220 MB on a first run); say so while you wait.
