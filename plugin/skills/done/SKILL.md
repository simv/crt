...
Close task `$0` (PRD F-40), from the project root `${CLAUDE_PROJECT_DIR}`. `crt` means `npx --no crt` if the project has it installed, else `npx -y claude-review-tool@latest`.
