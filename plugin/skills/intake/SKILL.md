...
4. **Propose the definition of done.** Reply with, in this order: one paragraph restating the ask (no heading, no list); the line `Proposed definition of done:`; the checklist of concrete, checkable DoD items (behavioural outcome, tests, no regressions), one `- [ ]` item per line; then exactly this line and nothing after it: `Accept as-is, or tell me what to change, and I'll write the task.` (the in-page chat folds the proposal to its first paragraph and the item count, and turns the line into an **Accept** button). Wait for the developer: a reply of `Accept` means go to step 5 with the proposal unchanged; anything else is an edit — apply it, propose again, and end with the same line.
