/**
 * Where the CRT server is (PRD F-6). Behind the proxy the overlay is same-origin and every API
 * path is relative. In script-tag mode the app loads `http://localhost:4400/__crt/overlay.js`
 * from its own origin, so the CRT origin is taken from the script's `src` (available as
 * `document.currentScript` while the bundle evaluates) and prefixed onto every request; the
 * server allows those cross-origin calls for localhost origins only.
 */
...
/** Empty behind the proxy; `http://localhost:<port>` in script-tag mode. */
export const CRT_ORIGIN = detectOrigin();
export const SCRIPT_TAG_MODE = CRT_ORIGIN !== "";
