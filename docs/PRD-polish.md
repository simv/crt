...
| | |
|---|---|
| **Status** | Built — M20–M24 landed (CRT-0024…0028, plus CRT-0029 for the shop screenshots, all 2026-09-22); §10 ticked with evidence by M24; release `0.6.0` is CRT-0028's last step |
| **Owner** | Simon (simv) |
...
Determinism: a fixed clock, cleared storage, the stub's fixed session ids (*M24 note: session and capture ids are minted by the server — `sessions.ts` `randomUUID()` — not by the stub, so the session id in `chat.png`'s footer is what a second run changes, 0.03 % of its pixels; the scratch project's path and the landing page's clock, Node, plugin and provider rows also vary between machines; `docs/images/README.md` lists them and N-27 accepts them — CRT-0026 Log 2026-09-22T11:35*), `animations: "disabled"` on capture, waits on the overlay's own signals (`data-health`, the pill's `data-state`) rather than fixed sleeps over 500 ms (N-27). `docs/images/README.md` says how to regenerate; the script never runs in CI (pixel diffs flake).
...
Each row names the milestone whose code applied it; "noted by M24" means the `*v0.6: …*` note inline in the earlier PRD was written by M24 (CRT-0028), the way M18 wrote the `*v0.4: …*` notes.

| Statement | v0.6 |
|---|---|
| PRD F-65 "inside the viewport" | inside the viewport and above the dock: when the toolbar is open in the lower half of the window, the popover is clamped above it (at 600 px a chat popover would otherwise end under the toolbar) — CRT-0029 (`ui.ts` `popoverViewport()`, e2e in `chat.spec.ts`); noted by M24. |
| PRD-embedded F-91 "a landing page that says, in plain words … and nothing else (no scripts)" | the F-114 page: the same sentences (the kicker now reads `This is the CRT server — not your app.`), the app link, plus Attention, the loop, Checkup and this server's facts; one inline script, same-origin only (N-23). The e2e/unit assertions `not.toMatch(/<script/)` become "exactly one inline `<script>` and no `src`" (M21); noted by M24. |
| PRD-embedded F-100 `crt init` footer "Production builds contain nothing from CRT (README › Production)" | "… (docs/integration.md › Production)" (M23); noted by M24. |
| PRD-embedded F-107 "README quotes every new line (doc test)" | the line is quoted in the README **or** in the `docs/` page the section moved to (§5.4); `docs.test.ts` is the doc test (M23); noted by M24. |
| PRD-setup F-80 line "… see README › Overlay does not appear" | "… see docs/troubleshooting.md › Overlay does not appear" (M23; the line stays one line, N-17); noted by M24. |
| PRD-setup F-88 / N-17 "quoted in the README verbatim" | README or `docs/troubleshooting.md` / `docs/cli.md` (M23); noted by M24. |
| PRD-providers F-62 / §10 "every N-7 provider line verbatim in the Providers section" | in `docs/providers.md`; the README keeps the providers table (M23); noted by M24. |
| PRD-setup F-76 `crt doctor` | unchanged on the terminal; its row text is also served by F-113 with the in-process differences named there (`port … — this server`) (M21); noted by M24. |
| PRD-setup F-78 health | unchanged; the landing page reads it (M21); noted by M24. |
| PRD-setup N-16 "No new network" | the landing page and F-113 add none: same-origin fetches and the existing localhost target probe (N-23, N-24) (M21); noted by M24. |
| PRD-providers F-64 "product chrome stays CRT" | holds on the landing page: the agent's display name appears in the loop card and the provider rows, the chrome says CRT (M21); noted by M24. |
| PRD §8 "Setup experience", §9 repository layout, §11 rows naming "README" | the README front page keeps the §8 block; the reference rows point at `docs/` (M23); noted by M24. |
| This document, F-114 "fetches only `/__crt/health`, `/__crt/providers`, `/__crt/doctor` and `/__crt/doctor?plugin=1`" | **Re-check** first fetches `/__crt/providers?refresh=1` (the PRD-providers F-56 preflight) so a `codex login` since the registry's last refresh shows in the rows; load and `visibilitychange` use the plain `/__crt/providers`; same-origin, no timer (N-23). Built by M21 (CRT-0025 Log 2026-09-22T09:30, decision b; flagged by the prd-reviewer as beyond the literal list); recorded inline in F-114 by M24. |
| CLAUDE.md "Read `docs/PRD.md`, … and `docs/PRD-embedded.md`" | "… `docs/PRD-embedded.md` and `docs/PRD-polish.md`" (applied in the PR that adds this document, #66). The prd-reviewer (`.claude/agents/prd-reviewer.md`) reads this document's §6, §7 and §9 too and knows F-112…F-118 / N-23…N-27 live here (M24, as M18 did for PRD-embedded). |
...
| CLAUDE.md Release | gains "re-run `npm run screenshots` when the overlay's UI changed since the last release" (M24; `docs/develop.md` › Release says the same). |

## 10. Definition of done (v0.6)

Ticked by M24 with evidence (test name, e2e spec, task Log entry or run URL).

- [x] The seven brand files exist, pass the SVG unit test, `tokens.ts` feeds the overlay with its CSS unchanged, and `GET /__crt/favicon.svg` serves the favicon (M20). — `test/brand.test.ts` (the `<svg>` root / `viewBox` / no `width`–`height` / no `<text>` `<image>` `<script>` `href` / `<style>` only in `favicon.svg` rows, the N-25 budgets — 361–902 bytes per SVG, `favicon.svg` 448 bytes — the palette, the dark files without `#111`, the badge geometry, the token values) and `brand.test.ts › renders the same declarations the literals did` — `OVERLAY_CSS` SHA-256 `7ed311e1…` identical before and after the token move (CRT-0024 Log 2026-09-22T00:05); `test/proxy.test.ts › serves /__crt/favicon.svg from next to the overlay bundle … (PRD-polish F-112)` and `… in embedded mode too`; `e2e/embedded.spec.ts`'s F-91 row asserts 200 `image/svg+xml` `max-age=86400` on the stub server; `npm run build` copies the favicon and the two PNGs into `dist/`; the tab-strip check in light and dark, `.crt/tasks/assets/CRT-0024/tab-strip-{light,dark}.png` (M20 Manual, 00:10). PR #68.
- [x] `http://localhost:4400/` on the trial app shows the app link, Attention when a row fails, the Checkup with passes collapsed, this server's facts and the three steps; `/__crt/` shows it in proxy mode; every request the page makes is same-origin (`e2e/landing.spec.ts`, M21 Manual). — CRT-0025 Log 2026-09-22T09:40 (trial app on `:3100`, the branch's dist on `:4400`, real Claude login): hero state 1 `Your app is at localhost:3100` + Open, state 2 `No dev server found yet.` from a scratch project, state 3's `Your page has no CRT integration yet — crt init prints the snippet.` after the F-94 timer with `<CrtDevTools />` removed; Attention with `warn target http://localhost:3100 (remembered) — not responding` after Next was stopped and `warn codex Codex CLI 0.154.0 — not logged in — codex login` under an empty `CODEX_HOME`, both gone after Re-check with `codex warn → ok` next to the time; the plugin row 524 ms after the fast rows. `e2e/landing.spec.ts`: the Checkup rows equal `/__crt/doctor`'s JSON (`4499 — this server`), the passes `<details>` opens, Re-check makes four requests, every request on the CRT origin (N-23), `animation-name: none` under `reducedMotion: "reduce"`, Tab open → recheck → docs with the accent ring, `#crt-host` absent, `/__crt/` on the proxy server `:4496` with `mode proxy` and `port 4496 — this server`. Accessibility pass 09:45: 113 text nodes per theme, none below AA (lowest 4.69:1 light, 5.81:1 dark), chips 6.25:1 / 7.12:1, rings 3.16–5.6:1; `.crt/tasks/assets/CRT-0025/landing-{light,dark,mobile}.png`. PR #70.
- [x] `GET /__crt/doctor` answers within 2 s without `?plugin=1`, refuses an `Origin`, caches 5 s / 30 s (unit rows, M21). — `test/doctor-route.test.ts` (the self `port` row, `target` `--`/ok/warn, `plugin` only with `?plugin=1` and its 30 s cache with a counted fake spawn, the 5 s cooldown with a fake clock, 503 without a registry, `Origin` → 403, 405 with `allow`, `HEAD`, ISO `checkedAt`); `e2e/landing.spec.ts` times the default route under 2 s on the stub server and sees the same `checkedAt` on a second call within 5 s; on the trial app the route answered in 619 ms, `?plugin=1` in +524 ms, a third call in 2 ms with the same `checkedAt` (CRT-0025 Log 2026-09-22T09:40, 09:55). PR #70.
- [x] `npm run screenshots` reproduces the five committed PNGs; the four SVGs render on GitHub in both themes (N-27, M22). — CRT-0026 Log 2026-09-22T11:35: four runs of `npm run screenshots` (5 passed, ~10 s each); the second pixel-diffed against the first — `arrival`, `select`, `marker-states` and `landing.png` byte-identical, `chat.png` 1262 px (0.03 %) in the popover footer's server-minted session id; what varies beyond rasterisation is listed in `docs/images/README.md` (§5.3 note); `test/docs-images.test.ts` (every referenced image exists, the five PNGs at 1600 × 1200 and ≤ 400 KB, the four SVGs ≤ 10 KB with a `viewBox` and no `currentColor` / `var(` / `<style>` / `href`); GitHub light and dark renders of both drawings as an anonymous visitor, `currentSrc` switching to the `-dark` files — `.crt/tasks/assets/CRT-0026/github-{loop,architecture}-{light,dark}.png` (M22 Manual, 11:41). Regenerated on `/shop` at 800 × 600 by CRT-0029 (Log 2026-09-22T13:22, 5/5, 143–217 KB each). Nothing under `packages/overlay/` changed between CRT-0029's images and the release commit (`git log e44e7a2..HEAD -- packages/overlay` empty, CRT-0028 Log), so the committed PNGs are the release's. PRs #72, #77.
- [x] The root README is ≤ 200 lines with the lockup, five screenshots and the loop illustration; the npm README ≤ 60 lines; every relative link resolves (M23, doc tests). — `test/docs.test.ts › the root README is ≤ 200 lines and the npm README ≤ 60 (F-115)` (198 / 57 lines at M23) and `… every relative link in the READMEs and docs/*.md resolves to a file, and to a heading when one is given (F-115)` (81 unique relative targets); the lockup `<picture>`, `arrival` / `select` / `chat` / `marker-states` / `landing.png` and `loop.svg` / `loop-dark.svg` present in `README.md` and checked by `test/docs-images.test.ts` (CRT-0027 Log 2026-09-22T12:40). PR #74.
- [x] Every assertion of the baseline `readme.test.ts` survives in `docs.test.ts` (N-26, M23 Log). — the comparison script and its output in CRT-0027 Log 2026-09-22T12:22: baseline 230 literals (main at 973b30c), 225 kept verbatim, 5 re-pointed (three section locators — `## Providers`, `` ### `crt` flags ``, `## How it works` — and the two server strings of §5.4), 0 missing; the prd-reviewer's independent AST diff of the two files found 0 unaccounted losses (13:02).
- [x] The README renders on GitHub in light and dark with the lockup switching (M23 Manual). — CRT-0027 Log 2026-09-22T12:50: github.com as an anonymous visitor (Playwright, 1280 px, DPR 2, `colorScheme` light then dark under `data-color-mode=auto`): the lockup's `currentSrc` `crt-lockup.svg` on light and `crt-lockup-dark.svg` on dark, `loop.svg` / `loop-dark.svg` likewise, the five screenshots inline, 81 relative targets opened one by one and all 200 with their anchors present — `.crt/tasks/assets/CRT-0027/github-readme-{hero,loop}-{light,dark}.png`; re-rendered after CRT-0029's images, `.crt/tasks/assets/CRT-0029/github-readme-*.png` (13:25).
- [x] CI green on `main`; `v0.6.0` tagged, published and promoted; the GitHub Release names M19–M23; the npm page shows the lockup (M24). — PR #80 green on both `check` runners, `e2e (ubuntu)` and CodeQL, squash-merged as dbb5f41 on `main`; tag `v0.6.0` at dbb5f41; `release` workflow run https://github.com/simv/crt/actions/runs/35696378582 succeeded 2026-09-22T06:48Z (`npm run check`, tag = package version, `npm stage publish … --provenance` → `claude-review-tool@0.6.0` staged with id 5ca1d973-6fc0-4be4-9d7b-9ae60fc95620, Sigstore logIndex 2908216288); GitHub Release https://github.com/simv/crt/releases/tag/v0.6.0 (Latest) with a summary naming M19 (Antigravity, #61), M20 (#68), M21 (#70), M22 (#72, #77), M23 (#74) and M24 (#80) above the generated PR list; Simon promoted the version 2026-09-22 ≈15:05+08 — `npm view claude-review-tool version` → `0.6.0`, `dist-tags.latest` 0.6.0; the npm page renders the lockup from `raw.githubusercontent.com/simv/crt/main/docs/brand/crt-lockup.svg` (300 × 113) above the Install block — `.crt/tasks/assets/CRT-0028/npm-page.png`; a clean `npm i -g claude-review-tool@0.6.0` + `crt setup` in a fresh profile installs `crt@crt 0.6.0` with the seven skills (CRT-0028 Log 2026-09-22T15:10, 15:15).
...
1. **The mark's direction: A (Tube)** — Simon's pick, 2026-09-21, from `docs/brand/options/` (the design review's first choice). The review's fixes (§5.1) apply. *Shipped by M20 (CRT-0024, PR #68) as `docs/brand/crt-mark.svg` and its six siblings: four cubic Béziers per superellipse centred at (60, 64) on the 128 grid, the badge (r = 16) on the point of maximum curvature with a 2-unit knock-out ring, the numeral a 7-unit stem with a wedge flag, the dark bezel `#f2f2f4`, the 16-grid small mark with a 4 px dot; the lockup's wordmark is drawn geometry, cap height 64. Wearing it since: the README hero and `packages/server/README.md` (M23), the landing page header and `/__crt/favicon.svg` (M21), the contact sheet's Final row.*
