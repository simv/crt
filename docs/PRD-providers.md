...
- **N-10 Windows first.** PATH/PATHEXT resolution, npm-shim parsing, `process.execPath` for our own shim, process-tree kill, LF everywhere, `spawn(..., { shell: false })` everywhere; nothing ever passes a `.cmd` to `spawn`. Unit and build jobs on `windows-latest` stay; the fake-`codex` e2e runs on both runners. *v0.3 (2026-09-16, Simon): the Windows e2e job was removed from CI to keep it fast; e2e runs on ubuntu only, and the Windows unit job (which drives the fake `codex` through the npm `.cmd` shim) is the Windows coverage of the shim path.*
