...
- **F-100 (Must) `crt init [--yes] [--no-instructions] [--snippet [--json]]`.** Explicit, itemised, idempotent. *v0.6: its footer's `(README › Production)` reads `(docs/integration.md › Production)` — PRD-polish §9, M23.* It prints the plan first — one line per item it would create or change, only the items not already in place:
  ```
